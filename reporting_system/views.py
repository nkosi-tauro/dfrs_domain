...
def employeeview(request, user_id):
    ...
    context = {"employee": employee, "user_id": user_id, "flaws": flaws_context}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                systemEvent.warning(
                    f"User with IP: {ip_address} has been rate limitted, {e.usage} login requests failed",
            ...
            systemEvent.warning(
                f"You have rate limitted a User with the IP: {ip_address}",
...
def reports_update(request, primary_key):
    ...
    They wont be able to modify any data,
    just mark the report as fixed or unfixed
    """
    report = ReportingForm2Model.objects.get(id=primary_key)
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            email = EmailMessage(
                subject, message, from_email="nkosilati23@gmail.com", to=[report.email]
            )
            # Initiate Thread
            EmailThread(email).start()

            systemEvent.info(
                f"The Public Report {report} has been updated.", initiator=request.user
            )
            return redirect("adminview")
    else:
    ...
    context = {"form": form, "request": request.user}
...
def employeeupdate(request, user_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            systemEvent.info(
                f"{employee} has updated their profile.", initiator=request.user
            )
...
def remove_flaw(request, user_id, flaw):
    ...
    context = {"user_id": user_id, "flaw_id": flaw}
